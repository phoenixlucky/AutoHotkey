...
Modify     = 修改 = 二值化后可以用鼠标在预览区点击手动修改黑白点
MultiColor = 多色查找 = 鼠标选择多种颜色，之后点击“确定”按钮
FindShape  = 寻找形状 = 单击多个颜色，它将基于第一种颜色进行二值化
Undo       = 撤销 = 撤销上一次选择的颜色
Undo2      = 撤销 = 撤销颜色列表中最后添加的颜色
...
Similar3   = 相似度 = 将颜色相似度调整为与所选颜色相等
AddColorSim  = 添加 = 将简单颜色添加到颜色列表并运行Color2Two
AddColorDiff = 添加 = 将差异颜色添加到颜色列表并运行Color2Two
ColorList  = = 用于转换黑白图像的颜色列表
...
s17 = 请先保存图片
s18 = 捕获|截图
s19 = 你确定要删除所有的截图吗?=单击多个颜色，它将基于第一种颜色进行二值化
    )"
